...
#   bucket = "sim-gamification-prod-tfstate"
#   versioning {
#     enabled = true
#   }
...
#   name         = "sim-gamification-prod-tfstate"
...
terraform {
  ...
  backend "s3" {
    bucket         = "sim-gamification-prod-tfstate"
    key            = "sim-infra-setup/terraform-templates/env/production/terraform.tfstate"
    region         = "ap-southeast-1"
    dynamodb_table = "sim-gamification-prod-tfstate-locks"
    encrypt        = false
  }
}
